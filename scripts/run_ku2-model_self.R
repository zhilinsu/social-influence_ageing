...
for (pop_value in pop_vec) {
	...
	model_file <- "scripts/stan_model/ku-model_self_2.stan"
	...
	for (s_block in c("1", "i", "p")) { # self blocks 
		...
		if (pop == 0) {
			file_name <- paste("stanfit_ku2_self_", s_block, ".rds", sep = "")
		} else if (pop == 1) {
			file_name <- paste("stanfit_ku2_ya_self_", s_block, ".rds", sep = "")
		} else if (pop == 2) {
			file_name <- paste("stanfit_ku2_oa_self_", s_block, ".rds", sep = "")
		}
