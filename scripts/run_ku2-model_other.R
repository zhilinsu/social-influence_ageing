...
# The script is for the Other blocks. 
...
for (pop_value in pop_vec) {
	...
	model_file <- "scripts/stan_model/ku-model_other_2.stan"
	...
	for (o_block in c("i", "p")) { # other blocks 
		...
		data_list <-  list(n_trials   = n_trials,
											 n_subjects = n_pt,
											 smallest   = 0,
											 largest    = 20, # refer to Michael's code 
											 s_dec      = self_decision,
											 o_rs       = reward_sooner,
											 o_rl       = reward_later,
											 o_dl       = delay_later)
		...
		if (pop == 0) {
			file_name <- paste("stanfit_ku2_other_", o_block, ".rds", sep = "")
		} else if (pop == 1) {
			file_name <- paste("stanfit_ku2_ya_other_", o_block, ".rds", sep = "")
		} else if (pop == 2) {
			file_name <- paste("stanfit_ku2_oa_other_", o_block, ".rds", sep = "")
		}
